...
def load_and_preprocess_image(path, img_size=(128, 128)):
    ...
    image = image / 255.0  # [0, 1] 범위로 정규화34
...
class Decoder(Model):
    ...
    def call(self, x):
        ...
        x = self.convT4(x)  # 해상도 128x128로 맞춤
...
def plot_latent_images(decoder, latent_dim, input_shape, n=5, scale=3.0, figsize=20):
    ...
    for i, yi in enumerate(grid_y):
        for j, xi in enumerate(grid_x):
            ...
            # 당근의 길쭉한 형태를 반영하기 위해 잠재 공간의 첫 두 차원에 강한 특징 부여
            z_sample[0, :2] = [xi * 1.5, yi * 0.5]  # 길쭉하고 수직으로 연장된 특징 반영
